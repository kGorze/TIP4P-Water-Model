...
# Script to launch VMD with the converted files
...
MODEL="tip4p"
TEMP="273"
VMD_DIR="$BASE_DIR/data/$MODEL/${TEMP}K/vmd"
MOVIE_DIR="$BASE_DIR/data/$MODEL/${TEMP}K/movie"
...
if [ ! -f "$VMD_DIR/md_final.pdb" ]; then
    ...
    echo "Please run the prepare_for_vmd.sh script first."
    exit 1
fi
