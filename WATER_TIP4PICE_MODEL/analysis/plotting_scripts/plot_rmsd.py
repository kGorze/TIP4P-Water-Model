...
def plot_rmsd_equilibrated(x, y, title, xlabel, ylabel, output_path):
    ...
    try:
        ...
        # Fallback to lower DPI if memory error occurs
        plt.savefig(output_path, dpi=100, bbox_inches='tight')
        plt.close()
        print(f"Warning: Memory error occurred. Saved plot with reduced DPI (100) to {output_path}")
    except Exception as e:
        print(f"Error saving RMSD plot: {e}")
