...
"""
Run all plotting scripts for the TIP4P/Ice water model analysis.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run all plotting scripts for TIP4P/Ice water model analysis")
    ...
    if os.path.exists(summary_script):
        ...
        if run_script(summary_script, analysis_dir, plots_dir, args.verbose):
            ...
            print(f"Summary report saved to {os.path.join(plots_dir, 'tip4pice_water_analysis_summary.png')}")
            print(f"Text summary saved to {os.path.join(plots_dir, 'tip4pice_water_analysis_summary.txt')}")
        else:
