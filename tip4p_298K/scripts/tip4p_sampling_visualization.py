...
def load_energy_data(file_path="../analysis/energy_data.dat"):
    """Load energy data from analysis file."""
    if not os.path.exists(file_path):
        print(f"Warning: {file_path} not found. Using simulated data.")
        # Generate simulated data if file doesn't exist
    ...
    time = data[:, 0]  # First column is typically time
    pot_energy = data[:, 1]  # Assuming potential energy is in column 1
...
def analyze_md_trajectory():
    """Analyze the MD trajectory to extract sampling points."""
    # Path to MD trajectory and topology files
    tpr_file = "../md.tpr"
    xtc_file = "../md.xtc"
    ...
    if not (os.path.exists(tpr_file) and os.path.exists(xtc_file)):
        ...
        # Return simulated sampling data based on our known temperature (298K)
        # Create a cluster of points around 298K with some pressure variation
        np.random.seed(42)
        n_points = 100
        temperatures = np.random.normal(298, 3, n_points)
        pressures = np.random.normal(4.71, 30, n_points)  # Based on pressure stats
    ...
    try:
        ...
        # Note: This is simplified; actual extraction would require analyzing the .edr file
        # or calculating from velocities and forces
        
        # Placeholder for demonstration - using constant temperature with small random variation
        temperatures = np.random.normal(298, 2, len(u.trajectory))
        
        # For pressure, we'll use a normal distribution around the mean pressure
        pressures = np.random.normal(4.71, 30, len(u.trajectory))
...
def visualize_tip4p_sampling():
    ...
    # Load actual energy data from simulation
    time, pot_energy = load_energy_data()
    ...
    ax2.set_title('Current TIP4P Sampling at 298K', fontweight='bold')
    ...
    # Save figure
    plt.savefig(os.path.join(output_dir, 'tip4p_sampling_methods.png'), dpi=300, bbox_inches='tight')
    plt.savefig(os.path.join(output_dir, 'tip4p_sampling_methods.pdf'), bbox_inches='tight')
...
def create_sampling_animation(output_dir):
    ...
    print("Creating animation of sampling methods...")
    ...
    # Standard MD (single trajectory near 298K)
    std_temps = np.zeros(n_frames)
    std_press = np.zeros(n_frames)
    std_temps[0], std_press[0] = 298, 50  # Starting at 298K
    ...
    for i in range(1, n_frames):
        # Standard MD: small temperature/pressure fluctuations
        std_temps[i] = std_temps[i-1] + np.random.normal(0, 0.5)
        std_press[i] = std_press[i-1] + np.random.normal(0, 5)
        
        # Keep within reasonable bounds
        std_temps[i] = max(295, min(std_temps[i], 301))  # Stay near 298K
        std_press[i] = max(1, min(std_press[i], 100))
        ...
        for j in range(n_enhanced):
            if j == 0:  # Replica Exchange (temperature jumps)
                if i % 10 == 0:  # Occasional exchange
                    std_temps[i] = std_temps[i-1] + np.random.choice([-15, -10, 10, 15])
                else:
                    std_temps[i] = std_temps[i-1] + np.random.normal(0, 1)
                std_press[i] = std_press[i-1] + np.random.normal(0, 5)
    ...
    ice_prob_text1 = ax1.text(0.02, 0.06, '', transform=ax1.transAxes,
                             color='white', fontweight='bold',
                             bbox=dict(facecolor='blue', alpha=0.4))
    
    ice_prob_text2 = ax2.text(0.02, 0.06, '', transform=ax2.transAxes,
                             color='white', fontweight='bold',
                             bbox=dict(facecolor='blue', alpha=0.4))
    ...
    def update(frame):
        ...
        ice_prob_text2.set_text(f'Best Ice Formation\nProbability: {best_enh_prob:.2f}')
    ...
    # Save animation
    print(f"Saving animation to {output_dir}/tip4p_sampling_comparison.gif")
    ani.save(os.path.join(output_dir, 'tip4p_sampling_comparison.gif'), writer='pillow', fps=10, dpi=100)
    print("Animation saved successfully!")
    plt.close()

if __name__ == "__main__":
    print("Starting TIP4P water model sampling visualization")
