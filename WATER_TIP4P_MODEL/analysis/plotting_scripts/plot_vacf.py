...
def calculate_power_spectrum(t, vacf, zero_padding=True, window=True):
    ...
    if window:
        # Blackman window provides better frequency resolution than Hann
        win = np.blackman(len(vacf))
    ...
    if zero_padding:
        # More aggressive zero padding for smoother spectrum
        n_pad = len(vacf) * 8
...
def plot_vacf_spectrum(x, y, title, xlabel, ylabel, legend_labels, output_path):
    """
    Create an improved vibrational spectrum plot from VACF data
    ...
    # Calculate the power spectrum with improved parameters
    # Use Blackman window for better frequency resolution
    win = np.blackman(len(vacf_data))
    vacf_windowed = vacf_data * win
    
    # Apply substantial zero padding for smoother spectrum
    n_pad = len(vacf_data) * 8  # More zero padding for better resolution
    vacf_padded = np.zeros(n_pad)
    vacf_padded[:len(vacf_data)] = vacf_windowed
    
    # Compute the FFT
    fft_result = np.fft.rfft(vacf_padded)
    
    # Get the corresponding frequencies
    dt = x[1] - x[0]  # Time step in ps
    freq_thz = np.fft.rfftfreq(len(vacf_padded), dt)  # in 1/ps = THz
    freq_cm1 = freq_thz * 33.356  # Convert to cm^-1
    ...
    # Create a figure with two subplots - full spectrum and low-frequency zoom
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10), dpi=300, gridspec_kw={'height_ratios': [1, 1]})
    
    # Plot the full spectrum (0-1000 cm⁻¹) in the top subplot
    # Limit to 1000 cm⁻¹ to focus on librational and translational modes
    full_freq_mask = freq_cm1 <= 1000
    if np.any(full_freq_mask):
        full_freq = freq_cm1[full_freq_mask]
        full_power = power[full_freq_mask]
        
        # Plot the spectrum with a more appropriate line style
        ax1.plot(full_freq, full_power, color='#1f77b4', linewidth=2.5)
        
        # Add vertical spans for known vibrational modes
        # Translational (collective) modes: 0-200 cm⁻¹
        ax1.axvspan(0, 200, alpha=0.2, color='#ffb6c1', 
                   label='Translational (0-200 cm⁻¹)')
        
        # Librational modes: 300-900 cm⁻¹
        ax1.axvspan(300, 900, alpha=0.2, color='#add8e6', 
                   label='Librational (300-900 cm⁻¹)')
        
        # Find and annotate major peaks in the spectrum
        peak_indices, _ = signal.find_peaks(full_power, height=0.3, distance=20)
        for idx in peak_indices:
            if idx < len(full_freq) and full_power[idx] > 0.3:  # Only annotate significant peaks
                ax1.annotate(f'{full_freq[idx]:.0f} cm⁻¹',
                            xy=(full_freq[idx], full_power[idx]),
                            xytext=(full_freq[idx], full_power[idx] + 0.1),
                            arrowprops=dict(facecolor='black', shrink=0.05, width=1, headwidth=5),
                            fontsize=10, ha='center')
        
        # Add VACF integral annotation (related to diffusion coefficient)
        if integral is not None:
            ax1.text(0.7, 0.85, f'VACF Integral: {integral:.3e}\n(Related to diffusion coefficient)',
                    transform=ax1.transAxes, fontsize=10, verticalalignment='top',
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
        
        ax1.set_xlabel('Frequency (cm⁻¹)', fontsize=12)
        ax1.set_ylabel('Power (normalized)', fontsize=12)
        ax1.set_title('Vibrational Spectrum', fontsize=14, fontweight='bold')
        ax1.grid(True, linestyle='--', alpha=0.4)
        ax1.legend(fontsize=10, loc='upper right')
        ax1.set_xlim(0, 1000)
        ax1.set_ylim(0, 1.05)
    
    # Plot the low-frequency part (0-200 cm⁻¹) in the bottom subplot
    # This focuses on the translational/collective modes
    low_freq_mask = freq_cm1 <= 200
    if np.any(low_freq_mask):
        low_freq = freq_cm1[low_freq_mask]
        low_power = power[low_freq_mask]
        
        # Apply scaling to make low-frequency features more visible
        # Use a different color to distinguish from the top plot
        ax2.plot(low_freq, low_power, color='#2ca02c', linewidth=2.5)
        
        # Add vertical spans for specific collective modes
        # Acoustic modes: 0-50 cm⁻¹
        ax2.axvspan(0, 50, alpha=0.2, color='#ffb6c1', 
                   label='Acoustic (0-50 cm⁻¹)')
        
        # Hydrogen-bond stretching: 50-120 cm⁻¹
        ax2.axvspan(50, 120, alpha=0.2, color='#add8e6', 
                   label='H-bond stretching (50-120 cm⁻¹)')
        
        # Hydrogen-bond bending: 120-200 cm⁻¹
        ax2.axvspan(120, 200, alpha=0.2, color='#d8f0d8', 
                   label='H-bond bending (120-200 cm⁻¹)')
        
        # Find and annotate peaks in the low-frequency region
        low_peak_indices, _ = signal.find_peaks(low_power, height=0.1, distance=10)
        for idx in low_peak_indices:
            if idx < len(low_freq) and low_power[idx] > 0.1:
                ax2.annotate(f'{low_freq[idx]:.0f} cm⁻¹',
                            xy=(low_freq[idx], low_power[idx]),
                            xytext=(low_freq[idx], low_power[idx] + 0.1),
                            arrowprops=dict(facecolor='black', shrink=0.05, width=1, headwidth=5),
                            fontsize=10, ha='center')
        
        ax2.set_xlabel('Frequency (cm⁻¹)', fontsize=12)
        ax2.set_ylabel('Relative Intensity (normalized)', fontsize=12)
        ax2.set_title('Low-Frequency Vibrational Spectrum (Collective Modes)', fontsize=14, fontweight='bold')
        ax2.grid(True, linestyle='--', alpha=0.4)
    ...
    # Add a caption with simulation details and explanation
    caption = (
        "TIP4P Water Model | Vibrational Analysis | VACF Fourier Transform\n"
        "The vibrational spectrum reveals characteristic modes of water molecules and their collective motions.\n"
        "Top: Full spectrum (0-1000 cm⁻¹) | Bottom: Low-frequency collective modes (0-200 cm⁻¹)"
    )
    fig.text(0.5, 0.01, caption, ha='center', fontsize=10, style='italic', alpha=0.7)
...
def main():
    ...
    if os.path.exists(vacf_spectrum_file):
        ...
        if len(x) > 0 and len(y) > 0:
            ...
            # Use the new plotting function for the spectrum
            plot_vacf_spectrum(x, y, plot_title, plot_xlabel, plot_ylabel, legend_labels, output_path)
    else:
