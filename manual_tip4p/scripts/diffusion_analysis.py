...
def calculate_msd_and_diffusion(universe, selection='name OW', msd_dimension='xyz', n_frames=None):
    ...
    try:
        ...
        print("Checking available attributes in msd_analyzer:")
        for attr in dir(msd_analyzer):
            if not attr.startswith('_'):
                print(f"  - {attr}")
        
        # Check if results attribute exists
        if hasattr(msd_analyzer, 'results'):
            print("msd_analyzer has 'results' attribute")
            # Check what's in results
            for attr in dir(msd_analyzer.results):
                if not attr.startswith('_'):
                    print(f"  - results.{attr}")
        
        # Try different ways to access the data based on MDAnalysis version
        if hasattr(msd_analyzer, 'results') and hasattr(msd_analyzer.results, 'timeseries'):
            ...
            if hasattr(msd_analyzer, 'lagtimes'):
                print("Using msd_analyzer.lagtimes for time")
                time_array = msd_analyzer.lagtimes
    ...
    # Convert time to ps if it's in different units
    time_array = time_array * universe.trajectory.dt
    ...
    # Define the region for fitting (200-1500 ps)
    fit_start_ps = 200
    fit_end_ps = 1500
    
    # Find the indices corresponding to the time range
    fit_start_idx = np.argmin(np.abs(time_array - fit_start_ps))
    fit_end_idx = np.argmin(np.abs(time_array - fit_end_ps))
    
    # Ensure we have enough points for fitting
    if fit_end_idx - fit_start_idx < 10:
        print("Warning: Not enough points in the 200-1500 ps range for reliable fitting")
        print("Falling back to using the entire dataset")
        fit_start_idx = 0
        fit_end_idx = len(time_array) - 1
    ...
    print(f"Diffusion coefficient: {D_standard:.4f} × 10⁻⁵ cm²/s (R² = {r_squared:.4f})")
    print(f"Fitted region: {fit_start_ps}-{fit_end_ps} ps")
    ...
    plt.plot(time_array, msd_array, 'b-', label='MSD')
    
    # Plot the linear fit over the fitted region
    fit_line = intercept + slope * fit_time
    plt.plot(fit_time, fit_line, 'r--', 
             label=f'Linear fit (200-1500 ps)\nD = {D_standard:.4f} × 10⁻⁵ cm²/s\nR² = {r_squared:.4f}')
    
    # Highlight the fitted region
    plt.axvspan(fit_start_ps, fit_end_ps, alpha=0.2, color='gray', label='Fitted region (200-1500 ps)')
    ...
    plt.plot(fit_time, fit_line, 'r--', 
             label=f'Linear fit (200-1500 ps)\nD = {D_standard:.4f} × 10⁻⁵ cm²/s\nR² = {r_squared:.4f}')
    plt.axvspan(fit_start_ps, fit_end_ps, alpha=0.2, color='gray')
    plt.xlabel('Time (ps)')
    plt.ylabel('MSD (Å²)')
    plt.title('Mean Squared Displacement vs. Time')
    plt.legend()
    ...
    plt.scatter(log_time, log_msd, s=10, alpha=0.7, c='blue')
    
    # Add reference lines for different regimes
    # Slope = 1 (ballistic regime)
    x_range = np.linspace(min(log_time), max(log_time), 100)
    plt.plot(x_range, x_range + log_msd[0] - log_time[0], 'g--', label='Slope = 1 (Ballistic)')
    
    # Slope = 0.5 (sub-diffusive regime)
    plt.plot(x_range, 0.5 * x_range + log_msd[0] - 0.5 * log_time[0], 'y--', label='Slope = 0.5 (Sub-diffusive)')
    
    # Slope = 1 (diffusive regime)
    plt.plot(x_range, x_range + log_msd[-1] - log_time[-1], 'r--', label='Slope = 1 (Diffusive)')
    
    # Highlight the fitted region in log scale
    log_fit_start = np.log10(max(fit_start_ps, min(time_array[valid_indices])))
    log_fit_end = np.log10(min(fit_end_ps, max(time_array[valid_indices])))
    plt.axvspan(log_fit_start, log_fit_end, alpha=0.2, color='gray', label='Fitted region (200-1500 ps)')
    
    plt.xlabel('Log Time (ps)')
    plt.ylabel('Log MSD (Å²)')
    plt.title('Log-Log Plot of MSD vs. Time (to identify diffusive regime)')
    plt.legend()
    plt.grid(True, alpha=0.3)
