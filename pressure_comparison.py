...
TARGET_PRESSURE = {
    "tip4p_273K": 1.0,      # Standard pressure
    "tip4p_298K": 1.0,      # Standard pressure
    "iteration_5_TIP4P": 1.0  # Standard pressure
}
...
def main():
    ...
    for system in systems:
        ...
        ax.plot(system_data['Time (ps)'], system_data['Pressure'], 
    ...
    for system in systems:
        ...
        smooth_time, smooth_pressure = calculate_running_average(
            system_data['Time (ps)'].values, system_data['Pressure'].values, window=200)
        ax.plot(smooth_time, smooth_pressure, color=COLOR_MAP[system], 
                linewidth=2.5, alpha=0.8, linestyle='-')
    
    # Add horizontal line for target pressure (usually 1 bar)
    ...
    ax.set_title('Water Pressure Comparison Over Time', fontsize=14, fontweight='bold')
    ...
    # Plot smoothed data in the inset for clarity
    for system in systems:
        system_data = combined_data[combined_data['System'] == system]
        zoom_data = system_data[system_data['Time (ps)'] >= zoom_start]
        
        # Calculate running average for the zoomed region
        if len(zoom_data) > 0:
            smooth_time, smooth_pressure = calculate_running_average(
                zoom_data['Time (ps)'].values, zoom_data['Pressure'].values, window=50)
            axins.plot(smooth_time, smooth_pressure, color=COLOR_MAP[system], linewidth=1.5)
    
    # Add target pressure line to inset
    axins.axhline(y=1.0, color='black', linestyle='--', alpha=0.7, linewidth=1.0)
    ...
    ax.annotate('Zoomed region', xy=(zoom_start, combined_data['Pressure'].min()),
               xytext=(zoom_start * 0.7, combined_data['Pressure'].min()),
    ...
    for system in systems:
        system_data = combined_data[combined_data['System'] == system]
        sns.kdeplot(system_data['Pressure'], label=system, fill=True, 
                   alpha=0.3, color=COLOR_MAP[system], ax=ax)
        
        # Add vertical line for mean pressure
        mean_pressure = system_data['Pressure'].mean()
        ax.axvline(x=mean_pressure, color=COLOR_MAP[system], 
                  linestyle='-', alpha=0.7, linewidth=1.5)
        
        # Add annotation for mean pressure
        ax.annotate(f"Mean: {mean_pressure:.2f} bar", 
                   xy=(mean_pressure, 0), 
                   xytext=(mean_pressure, 0.0001),
    ...
    ax.set_title('Water Pressure Distribution Comparison', fontsize=14, fontweight='bold')
    ...
    for system in systems:
        system_data = combined_data[combined_data['System'] == system]
        mean = system_data['Pressure'].mean()
        std = system_data['Pressure'].std()
        target = TARGET_PRESSURE.get(system, 1.0)
        deviation = mean - target
        stats_text += f"{system}: {mean:.2f} ± {std:.2f} bar (Δ from target: {deviation:.2f} bar)\n"
    ...
    try:
        ...
        sns.boxplot(x='System', y='Pressure', data=combined_data, ax=ax,
    ...
    for system in systems:
        ...
        sns.stripplot(x='System', y='Pressure', data=sampled_data, ax=ax,
                     size=3, color='black', alpha=0.2, jitter=True)
    
    # Add significance bars
    max_pressure = combined_data['Pressure'].quantile(0.95)  # Use 95th percentile to avoid outliers
    add_significance_bars(ax, combined_data, systems, max_pressure * 1.1)
    ...
    for system in systems:
        ...
        pressure = system_data['Pressure'].values
    ...
    ax.set_xlabel('Time (ps)', fontweight='bold')
    ax.set_ylabel('Cumulative Average Pressure (bar)', fontweight='bold')
    ax.set_title('Pressure Convergence Comparison', fontsize=14, fontweight='bold')
    ax.legend(title="System", loc='best', framealpha=0.7)
    
    # Add annotation about pressure convergence
    ax.text(0.02, 0.02, 
           "Note: In NPT simulations, pressure fluctuations are expected to be large.\n"
           "The cumulative average should converge to the target pressure (1.0 bar).",
    ...
    pressure_stats = combined_data.groupby('System')['Pressure'].agg(['mean', 'std', 'min', 'max', 'count']).reset_index()
    pressure_stats.columns = ['System', 'Mean Pressure (bar)', 'Std Dev (bar)', 
                             'Min Pressure (bar)', 'Max Pressure (bar)', 'Sample Count']
    
    # Add target comparison
    pressure_stats['Target Pressure (bar)'] = pressure_stats['System'].map(
        lambda x: TARGET_PRESSURE.get(x, 1.0))
    pressure_stats['Deviation from Target (bar)'] = pressure_stats.apply(
        lambda row: (row['Mean Pressure (bar)'] - row['Target Pressure (bar)']), axis=1)
    pressure_stats['Relative Deviation (%)'] = pressure_stats.apply(
        lambda row: ((row['Mean Pressure (bar)'] - row['Target Pressure (bar)']) / 
                    row['Target Pressure (bar)'] * 100), axis=1)
    ...
    with open(stats_file, 'w') as f:
        ...
        for i, sys1 in enumerate(systems):
            for j, sys2 in enumerate(systems):
                if i < j:
                    sys1_data = combined_data[combined_data['System'] == sys1]['Pressure'].values
                    sys2_data = combined_data[combined_data['System'] == sys2]['Pressure'].values
                    t_stat, p_value = scipy_stats.ttest_ind(sys1_data, sys2_data, equal_var=False)
        ...
        for _, row in pressure_stats.iterrows():
            ...
            f.write(f"  Mean Pressure: {row['Mean Pressure (bar)']:.4f} bar\n")
            f.write(f"  Std Deviation: {row['Std Dev (bar)']:.4f} bar\n")
            f.write(f"  Min Pressure: {row['Min Pressure (bar)']:.4f} bar\n")
            f.write(f"  Max Pressure: {row['Max Pressure (bar)']:.4f} bar\n")
            f.write(f"  Sample Count: {row['Sample Count']}\n")
            f.write(f"  Target Pressure: {row['Target Pressure (bar)']} bar\n")
            f.write(f"  Deviation from Target: {row['Deviation from Target (bar)']:.4f} bar\n")
            f.write(f"  Relative Deviation: {row['Relative Deviation (%)']:.4f}%\n")
            
            # Add note about pressure fluctuations
            rel_std = (row['Std Dev (bar)'] / abs(row['Mean Pressure (bar)'])) * 100
        ...
        f.write("Only the average pressure should converge to the target pressure (1.0 bar).\n")
        f.write("Standard deviations of hundreds or thousands of bar are typical for water systems.\n")
